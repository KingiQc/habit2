...
export default function Signup() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    setError("");
    if (!name || !age || !email || !password) { setError("Please fill in all fields"); return; }
    if (password.length < 6) { setError("Password must be at least 6 characters"); return; }
    setLoading(true);
    const err = await signup({ name, age: parseInt(age), email, password });
    setLoading(false);
    ...
    <div className="min-h-screen flex items-center justify-center bg-background px-6">
      <div className="auth-container animate-fade-in">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <input type="number" value={age} onChange={(e) => setAge(e.target.value)} placeholder="Age" min="1" max="120"
            className="w-full bg-secondary text-foreground rounded-xl px-4 py-3.5 outline-none focus:ring-2 focus:ring-muted-foreground/30 transition-all placeholder:text-muted-foreground" />
